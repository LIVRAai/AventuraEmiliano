...
function safeTutorPayload(body = {}) {
  ...
  return {
    ...
    missionNumber: Math.max(
      1,
      Math.min(999, Number(body.missionNumber) || 1)
    ),
    ...
    attemptCount: Math.max(
      0,
      Math.min(20, Number(body.attemptCount) || 0)
    ),
...
/*
  NOVA debe verse como una conversación normal.

  Este filtro elimina formatos como:
  ...
  *texto*
  ...
  porque el chat de la web muestra texto plano.
*/
...
export async function POST(request) {
  ...
  if (!lesson.question) {
    return json(
      {
        error: 'Escribe una pregunta para NOVA.'
      },
      400
    );
  }
  ...
  const transcript = lesson.history.length
    ? lesson.history
        .map(
          (m) =>
            `${m.role === 'assistant' ? 'NOVA' : 'Emiliano'}: ${m.content}`
        )
        .join('\n')
  ...
  try {
    ...
    if (!openaiResponse.ok) {
      ...
      return json(
        {
          error:
            'NOVA no pudo conectarse con la IA en este momento.'
        },
        502
      );
    }
    ...
    if (!message) {
      return json(
        {
          error:
            'NOVA recibió una respuesta vacía.'
        },
        502
      );
    }
